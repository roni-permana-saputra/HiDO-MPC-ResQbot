...
class MPC_single_shooting_multi_objective():
    def __init__(self, T, N, v_max, v_min, w_max, w_min, const_line):
        # Parameters
        # -----------------------------------
        # Initialisation 
        #   Init all parameters
    ...
    def _form_obj(self):
        ...
        for k in range(self.N):
            ...
            self.obj = self.obj+self.aaa*dist2line+self.bbb*dist_obj+self.ccc*angle_diff+self.ddd*mtimes(mtimes(transpose((st[0:3]-self.P[5:8])),Q),(st[0:3]-self.P[5:8]))+self.eee*mtimes(mtimes(transpose(st[3:5]),R),st[3:5])+self.fff*mtimes(mtimes(transpose(con),R),con)
