    ...
    def _form_obj(self):
        ...
        if self.mpc_type==0:
            ...
            for k in range(self.N):
                ...
                self.obj = self.obj+self.ddd*angle_diff+self.eee*mtimes(mtimes(transpose(st[3:5]),R),st[3:5])+self.fff*mtimes(mtimes(transpose(con),R),con)
            ...
            for k in range(self.N):
                ...
                self.obj = self.obj+self.ggg*dist_obj+self.hhh*mtimes(mtimes(transpose(st[3:5]),R),st[3:5])+self.iii*mtimes(mtimes(transpose(con),R),con)
