...
class MPC_single_shooting_multi_objective():
    def __init__(self, map, init_vehicle, casualty, obs_list, T, N, v_max, v_min, w_max, w_min, mpc_type):
        # Parameters
        # -----------------------------------
        # Initialisation 
        #   Init all parameters
        ...
        # Weight
        self.aaa = 17.60207797513348
        self.bbb = 23.8175407396662
        self.ccc = 87.44304649551955
        self.ddd = 76.44197574428617
        self.eee = 38.22699081922008
        self.fff = 79.19167138147495

        self.ggg = 77.37539279241437
        self.hhh = 86.61976136191322
        self.iii = 29.830235862466818
    ...
    def _form_obj(self):
        Q = SX.zeros(3,3); 
        Q[0,0] = 1;   Q[1,1] = 1;  Q[2,2] = 0.5; 

        R0 = SX.zeros(2,2)
        R0[0,0] = 0.1;  R0[1,1] = 0.1 

        R1 = SX.zeros(2,2)
        R1[0,0] = 0.1;  R1[1,1] = 0.01

        R2 = SX.zeros(2,2)
        R2[0,0] = 0.1;  R2[1,1] = 0.01
        ...
        if self.mpc_type==0:
            for k in range(self.N):
                ...
                self.obj = (self.obj+0.5*36.56724647970869*self.cal_dist2line_cost(st[0:3], target[0:3])
                                    +36.56724647970869*self.cal_dist2target_cost(st[0:3], target[0:3])
                                    +0.75*36.56724647970869*self.cal_dist2endline_cost(st[0:3], target[0:3])
                                    +0.1*30.258826676004698*self.cal_angleDiff_cost(st[2], target[2])
                                    +0.1258826676004698*mtimes(mtimes(transpose(con),R0),con)
                                    +obs_obj
                                    +10*mtimes(mtimes(transpose((st-self.P[3:6])),Q),(st-self.P[3:6])))
            ...
            for k in range(self.N):
                ...
                for j in range(len(gcrx)):
                    for obs in self.filtered_obs:
                        ...
                        bool_weight = 1/(1+np.exp(60*(dist)))
                        obs_obj =  obs_obj + bool_weight*150#*(1/(dist**2))
                self.obj = (self.obj+10*68.01501281612002*self.cal_dist2line_cost(st[0:3], target[0:3])
                                    +0.1*68.01501281612002*self.cal_dist2target_cost(st[0:3], target[0:3])
                                    +0.01*68.01501281612002*self.cal_dist2endline_cost(st[0:3], target[0:3])
                                    +10*60.088582798687845*self.cal_angleDiff_cost(st[2], target[2])
                                    +0*96.78399611800188*mtimes(mtimes(transpose(con),R1),con)
                                    +obs_obj
                                    +0*mtimes(mtimes(transpose((st-self.P[3:6])),Q),(st-self.P[3:6])))
        elif self.mpc_type==2:
            for k in range(self.N):
                st = self.X[:,k]  
                con = self.U[:,k]
                target = self.P[3:6]
                self.obj = (self.obj+1*98.01002847504917*self.cal_dist2line_cost(st[0:3], target[0:3])
                                    +1*93.03643454111301*self.cal_dist2target_cost(st[0:3], target[0:3])
                                    +10*95.28025019237253*self.cal_angleDiff_cost(st[2], target[2])
                                    +10*71.78399611800188*mtimes(mtimes(transpose(con),R2),con))

    def cal_angleDiff_cost(self, current_angle, target_angle):
    ...
    def calc_global_four_circles(self, st):
        ...
        gcrx = [(ix * np.cos(st[2]) + iy * np.sin(st[2])) +
              st[0] for (ix, iy) in zip(cr_x, cr_y)]
        gcry = [(ix * np.sin(st[2]) - iy * np.cos(st[2])) +
              st[1] for (ix, iy) in zip(cr_x, cr_y)]
        gcrr = cr_r
    ...
    def _form_const(self):
        ...
        if self.obs_list !=[]:
            ...
            print self.n_obstacles_to_circles_distances_constraints-self.n_map_y_constraints
            print len(self.filtered_obs)
