...
class MPC_single_shooting_multi_objective():
    def __init__(self, map, init_vehicle, casualty, obs_list, T, N, v_max, v_min, w_max, w_min, mpc_type):
        ...
        self.ddd = 96.51191382676383
        self.eee = 1.1258826676004698
        self.fff = 7.377948408873998
        self.ggg = 99.68796263590808
        self.hhh = 10.71050774307648
        self.iii = 92.6513994780735
        ...
        if self.mpc_type==0:
            ...
            for k in range(self.N):
                ...
                self.obj = (self.obj+10*68.01501281612002*self.cal_dist2line_cost(st[0:3], target[0:3])
                                    ...
                                    +0*96.78399611800188*mtimes(mtimes(transpose(con),R),con)
                                    +obs_obj
            ...
            for k in range(self.N):
                ...
                self.obj = (self.obj+1*98.01002847504917*self.cal_dist2line_cost(st[0:3], target[0:3])
                                    ...
                                    +10*95.28025019237253*self.cal_angleDiff_cost(st[0:3], target[0:3])
                                    +8*71.78399611800188*mtimes(mtimes(transpose(con),R),con))
        
    def cal_angleDiff_cost(self, current_angle, target_angle):
        # angleDiff = (numpy.arccos(np.cos(current_angle-target_angle)))**2
        # angleDiff = (np.pi - math.fabs(math.fmod(math.fabs(current_angle-target_angle), 2*np.pi) - np.pi))**2
